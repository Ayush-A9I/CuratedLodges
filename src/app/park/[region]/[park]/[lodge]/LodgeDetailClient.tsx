...
import { readLodgeHero, resolveLodgeHero } from '@/lib/lodgeHeroSection';
...
function mapApiToProfile(data: any) {
  ...
  return {
    ...
    lodgeHero: readLodgeHero(content.lodgeHero),
...
export default function LodgeDetailClient({ initialLodge, lodgeSlug }: LodgeDetailClientProps) {
  ...
  const slugFromRoute = (params?.lodge as string) || lodgeSlug;

  const [lodge, setLodge] = useState<LodgeDetail | null>(initialLodge);
  ...
  useEffect(() => {
    ...
    if (!slugFromRoute || initialLodge) return;

    setLoading(true);
    api.getLodgeBySlug(slugFromRoute)
      .then((data: any) => {
        setLodge(data);
        setError(null);
      })
  ...
  const resolvedHero = resolveLodgeHero(
    lodgeProfile.lodgeHero,
    lodge.thumbnail || '',
